...
def create_database(db_name='oakland'):
    ...
    create database with postGIS extensions.
...
def create_crime_table():
    ...
    Preprocess data via pre_process() from preprocessing_crime.py
...
def create_area_features():
    '''
    Output: Table with one row for each area, for each year,
            for each crime type as follows:

    For area/year/crime type
        (quality, nonviolent, vehicle_break_in, vehicle_theft):
    ...
    cur.execute('''
        ...
        CREATE TABLE area_features AS (
                ...
                WHERE d.Year < 2015 AND d.ogc_fid
                    NOT IN ('1820', '21161', '2023')
        ...
        DROP TABLE IF EXISTS area_features_two;
            ...
            CREATE TABLE area_features_two AS (
                WITH tmp_total AS (
                    SELECT ogc_fid,
                           year,
                           (q_count + nv_count + vbi_count + vt_count
                            + v_count)
                                AS total,
                           (q_hr0 + nv_hr0 + vbi_hr0 + vt_hr0 + v_hr0)
                                AS hr0_total
                    FROM area_features)
                SELECT a.ogc_fid, a.year,
                       t.total,
                       ...
                       (a.q_weekend + a.nv_weekend + a.vbi_weekend
                        + a.vt_weekend + a.v_weekend)
                            /(t.total-t.hr0_total) as weekend_percent,
                       (a.q_morning + a.nv_morning + a.vbi_morning
                        + a.vt_morning + a.v_morning)
                            /(t.total-t.hr0_total) as morning_percent,
                       (a.q_workday + a.nv_workday + a.vbi_workday
                        + a.vt_workday + a.v_workday)
                            /(t.total-t.hr0_total) as workday_percent,
                       (a.q_evening + a.nv_evening + a.vbi_evening
                        + a.vt_evening + a.v_evening)
                            /(t.total-t.hr0_total) as evening_percent
...
def create_area_features_three():
    '''
    Output: Table with one row for each area, for each year, as follows:

    For area/year:
    q_count: count of total that are quality crimes
    nv_count: count of total that are nonviolent crimes
    vbi_count: count of total that are vehicle break in crimes
    vt_count: count of total that are vehicle theft crimes
    v_count: count of total that are violent crimes
    weekend_percent: Percent of crimes occuring on the weekend
    morning_percent: Percent of crimes occuring in the morning (1-7am)
    workday_percent: Percent of crimes occuring in the workday (8am - 3pm)
    ...
    cur.execute('''
        ...
        DROP TABLE IF EXISTS area_features_three;

            CREATE TABLE area_features_three AS (
                WITH tmp_total AS (
                    SELECT ogc_fid,
                           year,
                           (q_count + nv_count + vbi_count
                            + vt_count + v_count)
                                AS total,
                           (q_hr0 + nv_hr0 + vbi_hr0 + vt_hr0 + v_hr0)
                                AS hr0_total
                ...
                SELECT a.ogc_fid, a.year,
                       ...
                       (a.q_workday + a.nv_workday + a.vbi_workday
                        + a.vt_workday + a.v_workday)
                            /(t.total-t.hr0_total) as daytime_percent,
                       (a.q_evening + a.nv_evening + a.vbi_evening
                        + a.vt_evening + a.v_evening)
                            /(t.total-t.hr0_total) as evening_percent
                       FROM area_features a
...
if __name__ == "__main__":
    ...
    if conn:
        ...
        print "Creating Third Feature Table"
        create_area_features_three()
